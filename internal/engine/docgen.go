...
// StdlibDocHeader is the header prepended to the generated doc/stdlib.md file.
const StdlibDocHeader = "<!-- Code generated by shac doc from stdlib.star. DO NOT EDIT. -->\n\n"
...
func genDoc(tmpdir, src, content string, isStdlib bool) (string, error) {
	...
	for _, node := range m.Nodes {
		// Upstream docstring.Parse downgrades an entire "Args:" block to a
		// free-form RemarkBlock if any unindented line in the block fails to match
		// "<name>: <desc>", which would otherwise silently render unformatted text.
		if docstring.Parse(node.Doc()).RemarkBlock("Args").Body != "" {
			return "", fmt.Errorf("in %s: %s has malformed Args section in docstring", src, node.Name())
		}
